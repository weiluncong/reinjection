...
void pycaseManager::loadPythonClass(const char *name)
{
    ...
    PyObject * pModule = PyImport_ImportModule(name);  //参数为Python脚本的文件名
    ...
    if (pModule)
    {
        ...
        PyObject* pClassCalc = PyDict_GetItemString(pDict, name);
        if (!pClassCalc) {
        ...
        //实例化类得到类对象
        pInstance_=PyObject_CallObject(pConstruct,NULL);
